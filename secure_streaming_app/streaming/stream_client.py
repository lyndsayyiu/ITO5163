...
The messages are generated by the DataSoure class and sent using AES-GCM encryption through secure_send().
"""
import socket
import json
import time
...
def start_client(host: str, port: int, identity: Identity):
    ...
    while True:
        ...
        message_dict = data_source.next_message()

        #Encode to JSON bytes before encryption
        message_json = json.dumps(message_dict).encode("utf-8")

        #Encrypt and send using AES-GCM
        secure_send(sock, session_key, message_json)
        print(f"[CLIENT] Sent: {message_dict}")
